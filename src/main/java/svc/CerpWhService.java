...
public int getMax4(String selectwh, String waringcode) {
	
	Connection con = getConnection();
	UnitedDAO CaptDao = UnitedDAO.getInstance();
	CaptDao.setConnection(con);	
	
	int sv = CaptDao.getMax4(selectwh, waringcode); 
	close(con); // �ڿ� ����
	return sv;
}
	...
	public boolean updateWhRefundTempStock(String waringcode, String itemcode,int count) {
		...
		boolean success = CaptDao.updateWhRefundTempStock(waringcode,itemcode,count); 
		close(con);
		return success;
	}
...
public ArrayList<wh> getWhSearchList(String searchoption,String firstdate,String finaldate,String progress,String searchvalue,String firstdate2,String finaldate2) {
	
	Connection con = getConnection();
	UnitedDAO CaptDao = UnitedDAO.getInstance();
	CaptDao.setConnection(con);	
	
	ArrayList<wh> whList = CaptDao.selectWhSearchList(searchoption,firstdate,finaldate,progress,searchvalue,firstdate2,finaldate2); 
	close(con); // �ڿ� ����
	return whList;
}
}
