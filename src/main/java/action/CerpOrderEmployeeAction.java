...
public class CerpOrderEmployeeAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request,
		...
		if(user != null) {
			...
			if(grade>=2) {
				...
				employeeList = logina.getEmployeeList();
				
				
				request.setAttribute("employee", employeeList);
				request.setAttribute("loginuser", user);
		
				ActionForward forward = new ActionForward("order_employee_popup.jsp", false); //  
				return forward;
			}
			request.setAttribute("loginuser", user);
