...
public class CerpWhTempUpdateOneFinishAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request,
		...
		if(user != null) {
			...
			if(grade>=2) {
				...
				success = logina.updateWhTempStock(waringcode,itemcode,count);
				
				
				if(success = true) {
					redirect=1;
					request.setAttribute("redirect", redirect);
					ActionForward forward = new ActionForward("warehousing_temp_update_one.jsp", false); //  
					return forward;
				}else {
					redirect=0;
					request.setAttribute("redirect", redirect);
